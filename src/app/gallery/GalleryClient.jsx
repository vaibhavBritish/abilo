...
import { CheckCircle, Globe, ShieldCheck, Star } from "@phosphor-icons/react";
  ...
  <section style={{ background: "var(--navy)", padding: "5rem 0 4rem" }}>
    <div className="container-custom">
      ...
      <p style={{ color: "#E5E7EB", fontSize: "1.1rem", maxWidth: 600, lineHeight: 1.7, margin: 0 }}>
        See our team in action — from careful packing to safe delivery across Canada. Every move tells a story.
      </p>
    </div>
  </section>
);
        ...
        <div className="container-custom">
          <div className="text-center mb-12 max-w-3xl mx-auto">
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-4 font-poppins">
             Our Work in Toronto & GTA
            </h2>
            <div className="w-16 h-1 bg-primary mx-auto mb-6 rounded-full" />
            <p className="text-gray-600 leading-relaxed">
              Explore our journey of delivering safety and trust. From careful packing
              of fragile items to seamless long-distance transportation, our gallery
              showcases the commitment and precision that defines every move
              we've handled across Canada.
            </p>
          </div>
          ...
          >
            ...
            `}</style>
            {[
              { value: "5,000+", label: "Moves Completed", icon: <CheckCircle size={32} weight="fill" /> },
              { value: "50+", label: "Cities Covered", icon: <Globe size={32} weight="fill" /> },
              { value: "100%", label: "Safe Delivery", icon: <ShieldCheck size={32} weight="fill" /> },
              { value: "4.9★", label: "Client Rating", icon: <Star size={32} weight="fill" /> },
            ].map(({ value, label, icon }, idx) => (
              <div key={idx} style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                <div style={{ fontSize: "1.75rem", marginBottom: 8, color: "#ff5a00" }}>{icon}</div>
