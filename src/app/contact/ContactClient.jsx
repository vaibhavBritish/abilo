  ...
  if (submitted) {
    return (
      <div className="bg-[#001219] border border-white/10 rounded-xl p-6 text-center">
        <PaperPlaneTilt size={32} className="mx-auto text-yellow-500 mb-4" />
        <h3 className="text-xl font-bold text-white mb-2">Message Sent!</h3>
        <button onClick={() => setSubmitted(false)} className="text-yellow-500">
          Send another
        </button>
      </div>
    );
    ...
    <div className="bg-[#001219] border border-white/10 rounded-xl p-5 md:p-6 shadow-2xl">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <input name="name" placeholder="Name" onChange={handleChange} required className="bg-black/20 border border-white/10 p-3 rounded-lg text-white outline-none focus:border-yellow-500 transition" />
          <input name="phone" placeholder="Phone" onChange={handleChange} required className="bg-black/20 border border-white/10 p-3 rounded-lg text-white outline-none focus:border-yellow-500 transition" />
        </div>
        <input name="email" placeholder="Email" onChange={handleChange} required className="bg-black/20 border border-white/10 p-3 rounded-lg text-white outline-none focus:border-yellow-500 transition" />
        <textarea name="message" rows={4} placeholder="Message" onChange={handleChange} required className="bg-black/20 border border-white/10 p-3 rounded-lg text-white outline-none focus:border-yellow-500 transition" />
        <button className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-3 rounded-lg flex justify-center items-center gap-2 transition-colors">
              ...
              {contacts.map((c) => {
                ...
                return (
                  <Wrapper
                    key={c.label}
                    href={c.href}
                    className="flex gap-3 bg-white p-4 rounded-xl border border-gray-100 hover:border-yellow-500/50 transition-colors shadow-sm"
                  >
                    <div className={`p-2 rounded bg-green-50 ${c.color}`}>
                      <c.icon size={18} />
                    </div>
                    <div>
